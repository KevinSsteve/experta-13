...
export const ProfitReportGenerator = () => {
  ...
  const generateReport = async () => {
    ...
    setLoading(true);
    try {
      const data = await getProfitReport(dateRange.from, dateRange.to, groupBy);
      setReportData(data);
      toast({
  ...
  const downloadReport = () => {
    ...
    a.href = url;
    a.download = `relatorio-lucros-${new Date().toISOString().slice(0, 10)}.csv`;
            ...
            <Table>
              ...
              <TableBody>
                {reportData.map((row, index) => (
                  <TableRow key={index}>
                    <TableCell>{groupBy === 'category' ? row.category : formatDate(row.period)}</TableCell>
